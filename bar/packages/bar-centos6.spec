...
mkdir packages
(
  cp %{_sourcedir}/packages/sqlite-src-*.zip packages
  (cd packages; unzip -q sqlite-src-*.zip)
  ln -s `find packages -maxdepth 1 -type d -name 'sqlite-src-*'|head -1` sqlite
)
(
  cp %{_sourcedir}/packages/icu4c-*.tgz packages
...
(
  cp %{_sourcedir}/packages/libgpg-error-*.tar.bz2 packages
  cp %{_sourcedir}/packages/libgcrypt-*.tar.bz2 packages
...
%{__make} OPTFLAGS="%{optflags}" -C bar %{?_smp_mflags}
...
%check
%{__make} test1-debug test2-debug test3-debug test4-debug test5-debug OPTIONS="--no-stop-on-attribute-error" COMPRESS_NAMES_LZMA="lzma1 lzma2 lzma3 lzma4 lzma5 lzma6 lzma7"
...
* Thu Oct 13 2016 Torsten Rupp <torsten.rupp@gmx.net> 0.20
  ...
  - fixed storage of split hardlink-entries!
...
* Fri Dec 17 2010 Torsten Rupp <torsten.rupp@gmx.net> 0.16
  ...
  - fix restoring split files which are read only
