...
/*
 * Flag values used by Tcl_ScanxObjCmd.
 */
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_ScanxObjCmd(dummy, interp, objc, objv)
    ...
    Tcl_Obj *CONST objv[];	/* Argument objects. */
{
...
#endif
    Tcl_UniChar ch, sch;
    char *quotedString, *quotedStringEnd, *stringChar;
	...
	switch (op) {
	    ...
	    case 'S':
		/*
		 * Scan a string up to width characters or whitespace. Handle " and '
		 */
                    ...
                    switch (sch) {
                        case '"':
                           while (*end != '\0') {
                               ...
                               memcpy(quotedStringEnd,stringChar,offset);
                               quotedStringEnd += offset;
                               if (--width == 0) {
                                   break;
                               }
                           }
		...
		if (!(flags & SCAN_SUPPRESS)) {
		    objPtr = Tcl_NewStringObj(quotedString, quotedStringEnd-quotedString);
		    Tcl_IncrRefCount(objPtr);
		    objs[objIndex++] = objPtr;
		}
		string = end;
