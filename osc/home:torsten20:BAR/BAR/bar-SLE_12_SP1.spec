...
%makeinstall DESTDIR=%{buildroot} DIST=1 SYSTEM=SuSE SYSTEM_INIT=systemd
install -d %{buildroot}/usr/sbin
ln -sf service %{buildroot}/usr/sbin/rcbarserver
...
%post
systemctl enable barserver.service 1>/dev/null 2>/dev/null
service barserver start 1>/dev/null || echo "Warning: Cannot start barserver with systemctl!"

%preun
service barserver stop 1>/dev/null || echo "Warning: Cannot stop barserver with systemctl!"
systemctl disable barserver.service 1>/dev/null 2>/dev/null
